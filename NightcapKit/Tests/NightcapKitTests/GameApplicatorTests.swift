...
final class GameApplicatorTests: XCTestCase {
    ...
    // MARK: - Regression: dxvk flag must not clobber an explicit backend

    @MainActor
    func testApplyExplicitBackendSurvivesDXVKFalse() throws {
        let bottle = try makeTestBottle()
        defer { cleanupBottle(bottle) }

        // `dxvk = false` writes `.recommended` through to the backend, so a
        // variant setting both used to end up on `.recommended` instead of the
        // backend it asked for.
        let variant = makeTestVariant(graphicsBackend: .wined3d, dxvk: false)
        let entry = makeTestEntry(variant: variant)

        _ = try GameConfigApplicator.apply(entry: entry, variant: variant, to: bottle)

        XCTAssertEqual(bottle.settings.graphicsBackend, .wined3d, "Explicit backend must win")
    }
