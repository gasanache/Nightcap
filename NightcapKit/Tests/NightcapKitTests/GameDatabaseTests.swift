...
final class GameDatabaseTests: XCTestCase {
    ...
    func testGameDBEntryDecodesFromJSON() throws {
        ...
        XCTAssertEqual(entry.provenance?.author, "Nightcap Team")
    }
    ...
    func testGameDBLoaderLoadsBundledJSON() {
        ...
        XCTAssertFalse(entries.isEmpty, "GameDB.json should contain at least one entry")

        // Structural invariants that hold at any catalogue size — the entry
        // count and which ratings appear both change as games are added.
        for entry in entries {
            XCTAssertFalse(entry.title.isEmpty, "Entry \(entry.id) has no title")
        }
