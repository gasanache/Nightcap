...
/// Diagnostic utilities for troubleshooting critical stability issues (frankea/Nightcap#40).
///
/// Goals:
...
public enum StabilityDiagnostics {
    ...
    public static func generateDiagnosticReport(
        ...
        var report = """
        ═══════════════════════════════════════════════════════
        Nightcap Stability Diagnostics Report
    ...
    private static func generateSystemInfo(config: Configuration) async -> String {
        ...
        if !appVersion.isEmpty {
            info += "Nightcap Version: \(appVersion)\(buildNumber.isEmpty ? "" : " (\(buildNumber))")\n"
        }

        if let nightcapWineVersion = NightcapWineInstaller.nightcapWineVersion() {
            let nightcapWineVersionString =
                "\(nightcapWineVersion.major).\(nightcapWineVersion.minor).\(nightcapWineVersion.patch)"
            info += "NightcapWine Version: \(nightcapWineVersionString)\n"
        } else {
            info += "NightcapWine Version: Not installed / unknown\n"
        }
