...
public struct BottleSettings: Codable, Equatable {
    ...
    /// Bottle-level environment variables, written by presets and the user.
    ///
    /// Resolved in the `bottleUser` layer, so these beat bottle/launcher
    /// defaults and lose to per-program settings.
    private var customEnvironment: [String: String] = [:]
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.customEnvironment = try container.decodeIfPresent(
            [String: String].self,
            forKey: .customEnvironment
        ) ?? [:]
    }
    ...
    /// for different applications. Nightcap targets Windows 11.
    ...
    /// Rockstar, EA App, Epic Games, and other platforms (frankea/Nightcap#41).
    ...
    /// to avoid steamwebhelper crashes (nightcap-app/nightcap#946, #1224, #1241).
    ...
    /// Bottle-level environment variables applied to every launch.
    public var environment: [String: String] {
        get { customEnvironment }
        set { customEnvironment = newValue }
    }
    ...
    public func populateBottleManagedLayer(
        ...
        // macOS Sequoia compatibility mode (nightcap-app/nightcap#1310, #1372)
        ...
        if sequoiaCompatMode {
            ...
            // This helps fix graphics corruption issues (nightcap-app/nightcap#1310)
            ...
            // Help with Steam and launcher compatibility (nightcap-app/nightcap#1307, #1372)
    ...
    ///
    /// This method implements the dual-mode launcher compatibility system from frankea/Nightcap#41:
    ...
    private func populatePerformancePreset(builder: inout EnvironmentBuilder) {
        ...
        case .performance:
            // Performance mode - prioritize FPS over visual quality (nightcap-app/nightcap#1361 fix)
