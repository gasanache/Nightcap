...
extension FileHandle {
    ...
    /// Writes a line to a Nightcap log file while enforcing the log size cap.
    ///
    ...
    /// Closes a Nightcap log file handle and clears any associated in-memory cap state.
    ...
    func writeApplicationInfo() {
        ...
        header += "Nightcap Version: \(Bundle.main.infoDictionary?["CFBundleShortVersionString"] ?? "")\n"
    ...
    func writeInfo(for bottle: Bottle) {
        ...
        if let version = NightcapWineInstaller.nightcapWineVersion() {
            header += "NightcapWine Version: \(version.major).\(version.minor).\(version.patch)\n"
        }
        ...
        if configured == .recommended {
            ...
            header += "Graphics Backend: \(resolved.displayName) (via Recommended)\n"
        } else {
            header += "Graphics Backend: \(configured.displayName)\n"
        }
        header += "D3DMetal Payload: \(NightcapWineInstaller.isD3DMetalInstalled())\n\n"
