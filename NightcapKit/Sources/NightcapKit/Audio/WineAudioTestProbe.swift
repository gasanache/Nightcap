...
/// Runs NightcapAudioTest.exe via Wine to exercise the audio stack.
///
    ...
    private func runTest(beep: Bool) async -> AudioProbeResult {
        guard let exeURL = testExeURL, FileManager.default.fileExists(atPath: exeURL.path) else {
            return AudioProbeResult(
                ...
                evidence: ["NightcapAudioTest.exe not found in app bundle"]
            )
        }
...
/// Internal model for parsing NightcapAudioTest.exe JSON output.
