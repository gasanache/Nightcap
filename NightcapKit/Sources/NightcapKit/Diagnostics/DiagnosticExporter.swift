...
public enum DiagnosticExporter {
    ...
    private static func generateSystemInfoSection() -> String {
        ...
        if let nightcapWineVersion = NightcapWineInstaller.nightcapWineVersion() {
            section += "- **NightcapWine:** \(nightcapWineVersion.major).\(nightcapWineVersion.minor)"
            section += ".\(nightcapWineVersion.patch)\n"
        }
        if let dxvkVersion = NightcapWineInstaller.nightcapWineDXVKVersion() {
            section += "- **DXVK:** \(dxvkVersion)\n"
        }
        if let dxmtVersion = NightcapWineInstaller.nightcapWineDXMTVersion() {
    ...
    static func generateSystemInfoJSON() async -> String {
        ...
        if let nightcapWineVersion = NightcapWineInstaller.nightcapWineVersion() {
            info["nightcapWineVersion"] =
                "\(nightcapWineVersion.major).\(nightcapWineVersion.minor).\(nightcapWineVersion.patch)"
        }
...
extension DiagnosticExporter {
    ...
    fileprivate static func buildZIPArchive(context: ZIPExportContext) async -> URL {
        ...
        let zipFilename = "Nightcap-Diagnostics-\(bottleName)-\(programName)-\(dateString).zip"

        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        let contentDir = tempDir.appendingPathComponent(
            "Nightcap-Diagnostics-\(bottleName)-\(programName)",
            isDirectory: true
        )
