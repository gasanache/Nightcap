...
public enum GameConfigApplicator {
    ...
    public static func apply(
        ...
        // 5. Apply the variant's environment variables onto the bottle so they
        // resolve in the bottleUser layer at launch. These used to be recorded
        // in the snapshot and never written, so they never took effect.
    ...
    public static func previewChanges(
        ...
        changes.append(contentsOf: environmentChanges(variant, bottle: bottle))

        changes.append(contentsOf: overlayChanges(settings, bottle: bottle))
    ...
    private static func applyVariantSettings(_ settings: GameConfigVariantSettings, to bottle: Bottle) {
        // `dxvk` writes through to the backend (false resets it to
        // `.recommended`), so it has to be applied before an explicit backend
        // or it silently clobbers it.
        ...
        applyOverlaySettings(settings, to: bottle)
    }
