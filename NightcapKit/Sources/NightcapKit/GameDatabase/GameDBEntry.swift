...
public struct TestedWith: Codable, Sendable, Equatable {
    ...
    /// The Nightcap version used during testing, if known.
    public let nightcapVersion: String?
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.nightcapVersion = try container.decodeIfPresent(String.self, forKey: .nightcapVersion)
...
public struct GameConfigVariantSettings: Codable, Sendable, Equatable {
    ...
    ///
    /// Resolves to whatever the active backend supports: the DXVK HUD on DXVK,
    /// and Wine's `fps` debug channel (logged, not overlaid) on wined3d, which
    /// has no overlay of its own.
    public let fpsCounter: Bool?
    /// Whether to enable Apple's Metal HUD. Draws only on Metal backends.
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.fpsCounter = try container.decodeIfPresent(Bool.self, forKey: .fpsCounter)
