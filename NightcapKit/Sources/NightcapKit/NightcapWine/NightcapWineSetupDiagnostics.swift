//
//  NightcapWineSetupDiagnostics.swift
//  NightcapKit
//
...
public struct NightcapWineSetupDiagnostics: Codable, Sendable {
    ...
    private func appendHeaderLines(into lines: inout [String], stage: String, error: String?) {
        lines.append("NightcapWine Setup Diagnostics (\(Self.issueURL))")
    ...
    private func appendVersionLines(into lines: inout [String]) {
        lines.append(contentsOf: Self.versionSectionLines(for: NightcapWineInstaller.nightcapWineInfo()))
    }
    ...
    /// path reads the installed record via `NightcapWineInstaller.nightcapWineInfo()`.
    /// The header is always emitted; the runtime/DXVK lines only when recorded.
    static func versionSectionLines(for info: NightcapWineVersion?) -> [String] {
        var lines = ["[VERSION]"]
        if let version = info?.version.description {
            lines.append("NightcapWine: \(version)")
        }
    ...
    private func appendDiskLines(into lines: inout [String]) {
        ...
        if let appSupport = Self.availableDiskString(for: NightcapWineInstaller.applicationFolder) {
